...
Запуск:
    ...
    python main.py --build-digest           # HTML-дайджест изменений за 60 дней
    python main.py --build-digest --use-ai-summary   # + AI-формулировки (нужен ANTHROPIC_API_KEY)
...
def main():
    ...
    group.add_argument("--build-digest", action="store_true",
                       help="собрать HTML-дайджест изменений за 60 дней "
                            "(без скана, из data/history.json)")
    ...
    parser.add_argument("--use-ai-summary", action="store_true",
                        help="формулировки новостей дайджеста через Anthropic API "
                             "(опционально: нужен ANTHROPIC_API_KEY и пакет anthropic)")
