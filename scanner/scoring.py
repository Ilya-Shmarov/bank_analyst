...
THRESHOLDS = {
    ...
    # компенсаций такси+ресторанов в месяц (безлимит = 5)
...
def _score_taxi_rest(text: str):
    ...
    if any(m in low for m in UNLIMITED_MARKERS):
        return "безлимит", 5
    counts = [int(n) for n in re.findall(r"(\d+)\s*в мес", low)]
    if counts:
        total = sum(counts)
        return f"{total} компенсаций/мес суммарно", _threshold_score(
            total, "taxi_rest_count")
